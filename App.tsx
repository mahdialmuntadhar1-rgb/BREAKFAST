...
const MainContent: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const bootstrap = async () => {
      ...
      if (data.session?.user && mounted) {
        ...
        const profile = await api.getOrCreateProfile(data.session.user, pendingRole);
  ...
  useEffect(() => {
    ...
    const loadPosts = async () => {
      ...
      try {
        const result = await api.getPosts({ governorate, limit: 8 });
        if (!mounted) return;
  ...
  const handleLoadMorePosts = async () => {
    if (!postsHasMore || !postCursor) return;
    const result = await api.getPosts({ governorate, cursor: postCursor, limit: 8 });
