...
export const PersonalizedEvents: React.FC<{ selectedGovernorate: string }> = ({ selectedGovernorate }) => {
  ...
  const fetchEvents = async (loadMore = false) => {
    setIsLoading(true);
    try {
      const result = await api.getEvents({ category: categoryMap[activeTab], governorate: selectedGovernorate, cursor: loadMore ? (cursor ?? 0) : 0 });
  ...
  useEffect(() => { fetchEvents(false); }, [activeTab, selectedGovernorate]);

  return (
    <section className="py-16"><div className="container mx-auto px-4"><h2 className="text-3xl font-bold text-white mb-8 text-center">{t('events.personalizedTitle')}</h2>
