...
export const SocialFeed: React.FC<SocialFeedProps> = ({ posts, isLoading, isLoggedIn, onLike, onComment, onShare, onLoginRequired }) => {
                            ...
                            <GlassCard className="p-0 overflow-hidden border-white/10 hover:border-white/20 transition-all duration-300 group">
                                ...
                                <div className="p-4 flex items-center justify-between bg-white/[0.02]">
                                    <div className="flex items-center gap-8">
                                        <button 
                                            onClick={() => isLoggedIn ? handleLike(post.id) : onLoginRequired?.()}
                                            disabled={!isLoggedIn}
                                        ...
                                        <button 
                                            onClick={() => isLoggedIn ? onComment?.(post.id) : onLoginRequired?.()}
                                            disabled={!isLoggedIn}
