        ...
        <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide">
          {stories.map((story) => (
            <div key={story.id} className="flex-shrink-0" onClick={() => setActiveStory(story as Story)}>
              <div className={`relative w-20 h-20 rounded-full p-0.5 cursor-pointer ${story.viewed ? 'bg-white/20' : 'bg-gradient-to-tr from-primary via-accent to-secondary'}`}>
          ...
          <div className="flex-shrink-0"><div className="relative w-20 h-20 rounded-full p-0.5 bg-white/20"><div className="w-full h-full rounded-full backdrop-blur-xl bg-dark-bg/80 flex items-center justify-center border-2 border-dashed border-white/30"><Plus className="w-8 h-8 text-white/50" /></div></div><p className="text-xs text-white/80 text-center mt-2 max-w-[80px] truncate">{t('stories.add')}</p></div>
        </div>
      </div>
