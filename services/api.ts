...
export const api = {
    ...
    async getDeals(params: { governorate?: string } = {}) {
        if (!isConfigValid) {
            let filtered = [...(mockData.deals || [])];
            if (params.governorate && params.governorate !== 'all') {
                filtered = filtered.filter(d => d.governorate?.toLowerCase() === params.governorate?.toLowerCase());
            }
            return filtered;
        }
        const path = 'deals';
        try {
            let q = query(collection(db, path), orderBy('createdAt', 'desc'), limit(10));
            if (params.governorate && params.governorate !== 'all') {
                q = query(q, where('governorate', '==', params.governorate));
            }
            const snapshot = await getDocs(q);
